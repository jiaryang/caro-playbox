...
RUN_TS_RE = re.compile(r"-(\d+\.\d+)-TP-\d+-")
TP_RANK_RE = re.compile(r"-TP-(\d+)-")
TOP_KERNELS = 8
...
def _pick_key(path: str) -> tuple:
    """Newest profile run wins; within a run, the lowest TP rank wins."""
    base = os.path.basename(path)
    m = RUN_TS_RE.search(base)
    run_ts = float(m.group(1)) if m else os.path.getmtime(path)
    m = TP_RANK_RE.search(base)
...
def discover(directory: str, pattern: str = DEFAULT_PATTERN) -> dict:
    ...
    Searches recursively: SGLang drops each profile run into its own timestamped
    subdirectory, and with TP>1 every rank writes a trace. All ranks step in
    lockstep, so one rank describes the step structure; keep rank 0 of the most
    recent run per concurrency. ``_cN`` tags in the basename give the
    concurrency; a single untagged file maps to conc=0.
    """
    found = {}
    untagged = []
    for path in sorted(glob.glob(os.path.join(directory, "**", pattern), recursive=True)):
        ...
        conc = int(m.group(1))
        prev = found.get(conc)
        if prev is None or _pick_key(path) > _pick_key(prev):
            found[conc] = path
    if not found and untagged:
        found[0] = max(untagged, key=_pick_key)
    if not found:
        raise ValueError(f"no traces matching {pattern} under {directory}")
    return found
...
def build_phase_scaling(pairs, label_a, label_b, phases) -> pd.DataFrame:
    ...
    for conc, a, b in pairs:
        for phase in phases:
            sa, sb = get_phase_stats(a.stats, phase), get_phase_stats(b.stats, phase)
            rows.append(
                {
                    "Concurrency": conc,
                    f"{label_a} bs": a.bs,
    ...
    for phase in labels:
        for label, idx in ((label_a, 1), (label_b, 2)):
            ...
            for conc, a, b in pairs:
                ...
                value = (
                    ...
                    else get_phase_stats(run.stats, phase).wall_ms
                )
        ...
        for conc, a, b in pairs:
            if phase == "decode total":
                ...
                va = get_phase_stats(a.stats, phase).wall_ms
                vb = get_phase_stats(b.stats, phase).wall_ms
...
def build_category_pivot(pairs, label_a, label_b, classifier, phase) -> pd.DataFrame:
    ...
    categories = set()
    for _, a, b in pairs:
        categories |= set(get_phase_stats(a.stats, phase).per_category_ms) | set(
            get_phase_stats(b.stats, phase).per_category_ms
        )
    rows = []
    for cat in sorted(categories, key=classifier.order_index):
        ...
        keep = False
        for conc, a, b in pairs:
            va = get_phase_stats(a.stats, phase).per_category_ms.get(cat, 0.0)
            vb = get_phase_stats(b.stats, phase).per_category_ms.get(cat, 0.0)
...
def build_top_kernels(pairs, label_a, label_b, classifier, phases) -> pd.DataFrame:
    ...
    for conc, a, b in pairs:
        for phase in phases:
            for run in (a, b):
                st = get_phase_stats(run.stats, phase)
...
def build_streams(pairs, label_a, label_b, phases) -> pd.DataFrame:
    ...
    for conc, a, b in pairs:
        for run in (a, b):
            ...
            totals = {}
            for phase in phases:
                for tid, ms in get_phase_stats(run.stats, phase).per_stream_ms.items():
